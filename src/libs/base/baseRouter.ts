...
import { RegExpOrString, TopologyDescriptionChangedEvent } from "mongodb"
    ...
    constructor(name: string, controller: any) {
        ...
        .get(async(req: Request, res: Response)=> {
            try {
                ...
                Logger.info(`${this.name}_GET_BY_ID_BEGIN`, 'request ended', response)
                res.status(200).json(response)
            } catch (error) {
        ...
        .delete(async(req: Request, res: Response)=> {
            try {
                ...
                Logger.info(`${this.name}_DELETE_BY_ID_BEGIN`, 'request ended', response)
                res.status(200).json(response)
            } catch (error) {
                Logger.error(`${this.name}_GET_BY_ID_ERROR`, 'internal server error', error)
                res.status(500).json(error)
